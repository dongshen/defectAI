...
public class ClusterEvaluation {
	...
	public static double Precision(int[] A, int[] B) {
		...
		int[] value = calculateTP_FP_TN_FN(A, B);
		int TP = value[0];
		int FP = value[1];
		int TN = value[2];
		int FN = value[3];
	...
	public static int[] calculateTP_FP_TN_FN(int[] A, int[] B) {
	...
	public static double RI(int[] A, int[] B) {
		...
		return (double) ((TP + TN) * 1.0 / (TP + FP + FN + TN));
	...
	public static double F_score(int[] A, int[] B) {
		...
		double P = TP * 1.0 / (TP + FP);
		double R = TP * 1.0 / (TP + FN);
		F_score = (beta * beta + 1) * P * R / (beta * beta * P + R);
		return F_score;
	}
